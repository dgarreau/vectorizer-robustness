...
for ex in examples:
    ...
    for current_length in range(2 * winsize + 1, t_max + 1):
        ...
        if implem == "gensim":
            q_orig_store[current_length - 2 * winsize - 1] = vectorizer.infer_vector(
                ex_current_list
            )
        elif implem == "scikit":
            q_orig_store[current_length - 2 * winsize - 1] = vectorizer.transform(
                [ex_current]
            ).todense()
        elif implem == "local":
        ...
        for i_simu in range(n_simu):
            ...
            if implem == "gensim":
                ...
                ] = vectorizer.infer_vector(ex_new_list)
            elif implem == "scikit":
                q_new_store[
                    current_length - 2 * winsize - 1, i_simu, :
                ] = vectorizer.transform([ex_new]).todense()
            elif implem == "local":
                q_new_store[
                    current_length - 2 * winsize - 1, i_simu, :
