...
def global_softmax(q_vec,
    ...
    if model == "PVDBOW":
        ...
        aux_h = np.dot(R_matrix,gcv) + repmat(np.dot(R_matrix,q_vec),T-2*winsize,1).T
        sm = softmax(aux_h,axis=0)
    else:
        print("Not implemented!")
    return sm

def global_psi(q_vec,
               example,
               R_matrix,
               model="PVDBOW",
               P_matrix=None,
...
def objective_helper(q_vec,
    ...
    """
    psi_wt = global_psi(q_vec,example,R_matrix,model=model,P_matrix=P_matrix,winsize=winsize)
    return np.sum(psi_wt)

def compute_objective(q_vec,
                      example_orig,
                      R_matrix,
                      model="PVDBOW",
                      P_matrix=None,
    ...
    """
    if mode == "true":
        obj = objective_helper(q_vec,example_orig,R_matrix,model=model,P_matrix=P_matrix,winsize=winsize)
    elif mode == "linear":
        obj_orig = objective_helper(q_vec,example_orig,R_matrix,model=model,P_matrix=P_matrix,winsize=winsize)
        obj_new = objective_helper(q_vec,example_new,R_matrix,model=model,P_matrix=P_matrix,winsize=winsize)
...
def gradient_helper(q_vec,
    ...
    """
    all_grad = global_gradient(q_vec,example,R_matrix,model=model,P_matrix=P_matrix,winsize=winsize)
    return np.sum(all_grad,axis=1)

def compute_gradient(q_vec,
                     example_orig,
                     R_matrix,
                     model="PVDBOW",
                     P_matrix=None,
    ...
    if model == "PVDBOW":
        if mode == "true":
            grad = (T-2*winsize)*np.dot(R_matrix.T,softmax(np.dot(R_matrix,q_vec))) - np.sum(R_matrix[example_orig[winsize:T-winsize],:],0)
    
        elif mode == "linear":
            grad_orig = (T-2*winsize)*np.dot(R_matrix.T,softmax(np.dot(R_matrix,q_vec))) - np.sum(R_matrix[example_orig[winsize:T-winsize],:],0)
            grad_new = (T-2*winsize)*np.dot(R_matrix.T,softmax(np.dot(R_matrix,q_vec))) - np.sum(R_matrix[example_new[winsize:T-winsize],:],0)
        ...
        if mode == "true":
            grad = gradient_helper(q_vec,example_orig,R_matrix,model=model,P_matrix=P_matrix,winsize=winsize)
        elif mode == "linear":
            grad_orig = gradient_helper(q_vec,example_orig,R_matrix,model=model,P_matrix=P_matrix,winsize=winsize)
            grad_new = gradient_helper(q_vec,example_new,R_matrix,model=model,P_matrix=P_matrix,winsize=winsize)
            grad = lbda*grad_new + (1-lbda)*grad_orig
        else:
            print("Not implemented!")
    else:
...
def compute_embedding(example_orig,
                     R_matrix,
                     model="PVDBOW",
                     P_matrix=None,
                     mode="true",
    ...
    if track_objective:
        obj_store[0] = compute_objective(q_vec,
                 example_orig,
                 R_matrix,
                 model=model,
                 P_matrix=P_matrix,
    ...
    for step in range(1,n_steps):
        ...
        if track_objective:
            obj_store[step] = compute_objective(q_vec,
                     example_orig,
                     R_matrix,
                     model=model,
                     P_matrix=P_matrix,
